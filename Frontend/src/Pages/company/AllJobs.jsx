...
const HeartIcon = ({ filled }) => (
    <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill={filled ? "#b91c1c" : "none"} stroke={filled ? "#b91c1c" : "currentColor"} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M20.84 4.61a5.5 5.5 0 0 0-7.78 0L12 5.67l-1.06-1.06a5.5 5.5 0 0 0-7.78 7.78l1.06 1.06L12 21.23l8.84-8.84 1.06-1.06a5.5 5.5 0 0 0 0-7.78z"></path></svg>
);
const ShareIcon = () => (
...
const AllJobs = () => {
    ...
    const handleShare = async (job) => {
        const shareData = {
            title: job.JobTitle,
            text: `Check out this job: ${job.JobTitle} at ${job.CompanyName}`,
            url: window.location.origin + `/jobs/${job.Company_URL?.replace(/^\/+/, "")}/${job.JobSlug}`,
        };
        try {
            if (navigator.share) {
    ...
    return (
        <div className="min-h-screen bg-slate-200 text-slate-800 font-sans relative">
...
<div className="flex flex-col md:flex-row justify-between items-center mb-8 border-b border-gray-100 pb-4">
    <div className="flex flex-col items-center md:items-start w-full"> {/* Container for Title + Button */}
        ...
        {/* NEW FILTER BUTTON: Appears under title on mobile only */}
        <button 
            onClick={() => setShowSidebar(true)}
            className="lg:hidden mt-4 flex items-center gap-2 bg-white border border-green-900 text-green-900 px-5 py-2 rounded-full font-medium shadow-sm active:scale-95 transition-all"
        >
            <FilterIcon />
            
        </button>
    </div>
    <div className="hidden md:block text-sm text-gray-500">Home / Jobs / Search</div>
</div>
                ...
                <div className="flex flex-col lg:flex-row gap-3">
                    ...
                    <aside className={`
                        ...
                        w-full sm:w-80 lg:w-1/5 p-4 lg:p-0
                    `}>
                        ...
                        <div className="bg-white p-6 rounded-xl shadow-xl border border-gray-200 space-y-6 h-full lg:h-auto overflow-y-auto">
                            ...
                            <div className="bg-gray-50 border border-gray-200 rounded-lg p-1">
                                <div className="relative">
                                    ...
                                    <input
                                        type="text"
                                        className="block w-full pl-9 pr-3 py-1 bg-transparent rounded-md leading-5 placeholder-gray-400 focus:outline-none focus:ring-0 sm:text-sm"
                            ...
                            <div className="space-y-3">
                                <div className="flex justify-between text-sm font-medium text-slate-700"><span>Salary Range</span></div>
                                <input type="range" className="w-full h-2 bg-gray-200 rounded-lg appearance-none cursor-pointer accent-green-900" />
                                <div className="flex justify-between text-xs text-gray-500"><span>$0</span><span>$10k+</span></div>
                            </div>

                            <div className="space-y-3 pt-2">
                                <div className="flex justify-between text-sm font-medium text-slate-700"><span>Experience</span></div>
                                <input type="range" className="w-full h-2 bg-gray-200 rounded-lg appearance-none cursor-pointer accent-green-900" />
                                <div className="flex justify-between text-xs text-gray-500"><span>0 yr</span><span>10 yr+</span></div>
                            </div>
                    ...
                    <main className="w-full lg:w-3/4">
                        {filteredJobs.length === 0 ? (
                            <div className="text-center py-20 bg-gray-50 rounded-xl border border-dashed border-gray-300">
                                <div className="inline-block p-4 rounded-full bg-white mb-4 shadow-sm"><SearchIcon /></div>
                                        ...
                                        <div key={job.JobID} className="group flex flex-col md:flex-row bg-white rounded-xl p-6 shadow-sm border border-gray-200 hover:shadow-lg hover:border-blue-200 transition-all duration-300">
                                            <div className="flex-1 pr-0 md:pr-6 ">
                                                <div className="flex justify-between items-start">
                                                    <div>
                                                        <Link to={`/jobs/${safeCompanyUrl}/${job.JobSlug}`} className="text-xl font-bold text-slate-800 group-hover:text-blue-600 transition-colors block mb-1">
                                                            {job.JobTitle}
                                                        </Link>
                                                        <div className="flex items-center text-sm text-gray-500 mb-3 gap-3">
                                                            <span className="flex items-center hover:text-blue-600 transition-colors">
                                                                <BuildingIcon />
                                                                <Link to={`/company/${safeCompanyName}/dashboard`}>{job.CompanyName || "N/A"}</Link>
                                                            </span>
                                                            <span className="flex items-center"><MapPinIcon />{job.City}, {job.Country}</span>
                                                        </div>
                                                    </div>
                                                    <span className="hidden sm:inline-flex items-center px-3 py-1 rounded-full text-xs font-medium bg-green-50 text-green-700 border border-green-100">{job.JobType || "Full Time"}</span>
                                                </div>
                                                <p className="text-sm text-gray-600 mb-4 line-clamp-2">{job.JobDescription}</p>
                                                <div className="flex flex-wrap items-center gap-2">
                                                    <span className="px-2 py-1 bg-gray-100 text-gray-600 text-xs rounded border border-gray-200 font-medium">{job.Experience || "Exp: N/A"}</span>
                                                    <span className="px-2 py-1 bg-blue-50 text-green-900 text-xs rounded border border-blue-100 font-bold">{job.SalaryFrom} - {job.SalaryTo} {job.Currency}</span>
                                                </div>
                                            </div>
                                            ...
                                            <div className="mt-4 md:mt-0 md:w-48 flex flex-col justify-center border-t md:border-t-0 md:border-l border-gray-100 pt-4 md:pt-0 md:pl-6">
                                                <button
                                                    ...
                                                    className="w-full bg-green-900 text-white font-semibold py-2 rounded-lg hover:bg-blue-700 transition-all duration-200 text-sm shadow-md"
                                                >
                                                    Apply Now
                                                </button>
                                                ...
                                                <div className="flex justify-center items-center gap-4 mt-4">
                                                    <button
                                                        ...
                                                        className="p-2 rounded-full hover:bg-red-50 transition-colors text-gray-400"
                                                        title="Favorite"
                                                    >
                                                        <HeartIcon filled={favorites.includes(job.JobID)} />
                                                    </button>
                                                    <button
                                                        onClick={() => handleShare(job)}
                                                        className="p-2 rounded-full hover:bg-blue-50 transition-colors text-gray-400"
                                                        title="Share"
                                                    >
                                                        <ShareIcon />
                                                    </button>
                        ...
                        {filteredJobs.length > 0 && (
                            <div className="mt-8 flex justify-center items-center gap-1">
                                <button onClick={prevPage} disabled={currentPage === 1} className={`px-4 py-2 rounded-md border text-sm font-medium transition-colors ${currentPage === 1 ? 'bg-gray-50 text-gray-300 border-gray-200 cursor-not-allowed' : 'bg-white text-gray-700 border-gray-300 hover:bg-gray-50'}`}>Previous</button>
                                <div className="flex gap-1">
                                    {Array.from({ length: totalPages }, (_, i) => i + 1).map((number) => (
                                        <button key={number} onClick={() => paginate(number)} className={`w-10 h-10 rounded-md border text-sm font-medium transition-colors ${currentPage === number ? 'bg-green-900 text-white border-blue-600 shadow-sm' : 'bg-white text-gray-700 border-gray-300 hover:bg-gray-50'}`}>{number}</button>
                                    ))}
                                </div>
                                <button onClick={nextPage} disabled={currentPage === totalPages} className={`px-4 py-2 rounded-md border text-sm font-medium transition-colors ${currentPage === totalPages ? 'bg-gray-50 text-gray-300 border-gray-200 cursor-not-allowed' : 'bg-white text-gray-700 border-gray-300 hover:bg-gray-50'}`}>Next</button>
                            </div>
                        )}
                    </main>
                </div>
            ...
            <ApplyModal isOpen={showModal} onClose={() => setShowModal(false)} job={selectedJob} />
        </div>
    );
};
